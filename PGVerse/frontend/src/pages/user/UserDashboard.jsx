...
const UserDashboard = ({ view }) => {
  ...
  const uniquePgs = Array.from(
    new Map(
      bookings.map((b) => [
        b.pgPropertId,
        { pgId: b.pgPropertId, pgName: b.pgPropertyName },
      ])
    ).values()
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingReviewId) {
        ...
        toastify.success("Review updated successfully!");
      } else {
        //Add Review
        await addReview(
          pgIdNum,
        ...
        toastify.success("Review added successfully!");
      }
      setRating(1);
      setComment("");
      setSelectedPgId("");
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    if (window.confirm("Are you sure you want to delete this review?")) {
      try {
        ...
        toastify.error("Failed to delete review.");
      }
    }
  };
  ...
  const handleViewBooking = async (bookingId) => {
    ...
    try {
      ...
      toastify.error("Failed to fetch booking details.");
    }
  };
          ...
          {reviews.length === 0 ? (
            ...
            <ul className="space-y-4">
              {reviews.map((review) => (
                ...
                >
                  <p>
                    <strong>PG:</strong> {review.pgPropertyName}
                  </p>
                  <p>
                ...
                {bookings.map((booking) => (
                  <tr key={booking.bookingId}>
                    ...
                    <td className="px-4 py-2 border">
                      ...
                      isCancellable(booking.bookingDate) ? (
                        <button
            ...
            <div className="fixed inset-0 flex items-center justify-center backdrop-blur-sm bg-black/30 z-50 transition-opacity duration-300 ease-in-out">
              <div className="bg-white rounded-2xl shadow-xl w-11/12 max-w-2xl p-6 relative transform transition-all duration-300 ease-in-out scale-100">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <strong>PG Property ID:</strong>{" "}
                    {selectedBooking.pgPropertId}
                  </div>
                  <div>
                    <strong>PG Name:</strong> {selectedBooking.pgPropertyName}
                  </div>
                  <div>
