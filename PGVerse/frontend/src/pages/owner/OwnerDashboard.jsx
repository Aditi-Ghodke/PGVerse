...
const OwnerDashboard = () => {
  ...
  const handlePgSubmit = async (e) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.message || "Failed to save PG Property";
      setPgError(errMsg);
    }
  };
  ...
  //ADD ROOM
    ...
    const handleRoomInputChange = (e) => {
      ...
      const val = ["capacity", "floor", "currentOccupancy", "pricePerMonth"].includes(name) ? Number(value) : value;
    ...
    setRoomUpdateMsg("");
      ...
      const handleAddService = async () => {
        try {
          ...
          setServiceAddError(error.response?.data?.message || "Failed to add service.");
          setServiceAddMsg("");
        }
      };
    ...
    try {
      ...
      const handleManuallyUpdateBookings = async () => {
        try {
          ...
          setUpdateBookingMsg(error.response?.data?.message || "Failed to update bookings");
        }
      };
...
const selectedPg = getPgByIdFromBookings(selectedPgId);
    ...
    <div className="p-4 max-w-4xl mx-auto space-y-10">
      ...
      {view === "dashboard" && ownerData && (
        <div className="bg-gray-100 p-4 rounded shadow">
          ...
          <p><strong>Gender:</strong> {ownerData.gender}</p>
          <p><strong>Address:</strong> {ownerData.address}</p>
          <p><strong>Card:</strong> {ownerData.card}</p>
        </div>
      )}

      {/* EDIT OWNER VIEW */}
      {view === "editOwner" && ownerData && (
        <form onSubmit={handleUpdateSubmit} className="bg-gray-100 p-4 rounded shadow space-y-4">
        ...
        <div className="bg-gray-100 p-4 rounded shadow space-y-4">
          ...
          {pgDetails && (
            <div className="bg-white p-4 border rounded space-y-1">
              <p><strong>PG ID:</strong> {pgDetails.pgId}</p>
              <p><strong>Name:</strong> {pgDetails.name}</p>
              <p><strong>Location:</strong> {pgDetails.location}</p>
              <p><strong>PG Type:</strong> {pgDetails.pgType}</p>
              <p><strong>Status:</strong> {pgDetails.status}</p>
              <p><strong>Description:</strong> {pgDetails.description}</p>
              <p><strong>Owner ID:</strong> {pgDetails.ownerid}</p>
              <p><strong>Owner Name:</strong> {pgDetails.ownername}</p>
 ...
 {/* UPDATE ROOM */}
  ...
  <div className="bg-white p-4 border rounded mt-4">
    ...
    />
    <button onClick={handleFetchRoomById} className="bg-blue-500 text-white px-4 py-2 rounded">
      Get Room
    </button>

    {fetchedRoom && (
      <div className="mt-4">
        <p><strong>Room Number:</strong> {fetchedRoom.roomNumber}</p>
        <p><strong>Floor:</strong> {fetchedRoom.floor}</p>
        <p><strong>Capacity:</strong> {fetchedRoom.capacity}</p>
        <p><strong>Price:</strong> ₹{fetchedRoom.pricePerMonth}</p>
        {/* Add more if needed */}
      </div>
    )}
    {roomActionMsg && <p className="text-red-500 mt-2">{roomActionMsg}</p>}
  </div>
)}
...
{view === "deleteRoom" && (
  <div className="bg-white p-4 border rounded mt-4">
    ...
    <input
      ...
      className="w-full p-2 border rounded mb-2"
    />
    <button onClick={handleDeleteRoom} className="bg-red-500 text-white px-4 py-2 rounded">
      Delete Room
    </button>
    {roomActionMsg && <p className="text-red-500 mt-2">{roomActionMsg}</p>}
  </div>
  ...
  <div className="bg-white p-4 rounded shadow mt-4">
   ...
   <label className="block mb-1">Room</label>
    ...
    {serviceAddMsg && <p className="text-green-600 mt-2">{serviceAddMsg}</p>}
    {serviceAddError && <p className="text-red-600 mt-2">{serviceAddError}</p>}
  </div>
)}
